...
class StudentSyncService extends BaseSyncService
{
    ...
    protected array $facultyConfig = [
        286 => 'หมวดวิชาศึกษาทั่วไป (3/2568)',
        291 => 'คณะครุศาสตร์ (3/2568)',
        290 => 'คณะมนุษยศาสตร์และสังคมศาสตร์ (3/2568)',
        287 => 'คณะวิทยาศาสตร์และเทคโนโลยี (3/2568)',
        288 => 'คณะวิทยาการจัดการ (3/2568)',
        291 => 'คณะพยาบาลศาสตร์ (3/2568)',
        294 => 'โรงเรียนการเรือน (3/2568)',
        293 => 'โรงเรียนการท่องเที่ยวและการบริการ (3/2568)',
        292 => 'โรงเรียนกฎหมายและการเมือง (3/2568)',
    ];
    ...
    protected function getExistingDataByFaculty(int $facultyId): array
    {
        try {
            ...
            $data = $this->targetDb->table($this->targetTable)
                ->where('CAT_TERM_ID', $facultyId)
                ->get()
                ->getResultArray();

            // Add hash to each record (calculate on-the-fly)
            foreach ($data as &$row) {
                $row[$this->hashColumn] = $this->calculateRecordHash($row);
            }
    ...
    /**
     * Build INSERT SQL
     * Note: RECORD_HASH is not stored in Oracle (used only for comparison)
     */
    private function buildInsertSQL(array $row): string
    {
        // Remove hash column - don't store it in Oracle
        ...
        // DON'T add RECORD_HASH to Oracle table
        // It's calculated on-the-fly for comparison only
    ...
    private function buildUpdateSQL(array $row): string
    {
        ...
        // DON'T update RECORD_HASH in Oracle
        // It's calculated on-the-fly for comparison only
